            ...
            foreach (var s in line) {
                try {
                    ...
                    switch (s) {
                        ...
                        case "2": work = new Space(); Player p = new Player(mMediator); p.SetPosition(new Vector2(colCnt * GridSize.GRID_SIZE, lineCnt * GridSize.GRID_SIZE)); p.GetMove().AddPlayerFootprint(p.GetPosition()); break;
                        case "3": work = new Space(); Box b = new Box(mMediator); b.SetPosition(new Vector2(colCnt * GridSize.GRID_SIZE, lineCnt * GridSize.GRID_SIZE)); b.AddFootprint(b.GetPosition()); break;
